...
public class MainActivityTest {
    ...
    public void testActivitySwitch() {
        ...
        // onData(is("Edmonton")) finds the data "Edmonton" in the adapter
        onView(withText("Edmonton")).perform(click());

        // Check if ShowActivity is displayed by checking for its "BACK" button
        onView(withId(R.id.button_back)).check(matches(isDisplayed()));
    }

    @Test
    public void testCityNameConsistency() {
        // Add a different city
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Vancouver"));
        onView(withId(R.id.button_confirm)).perform(click());
    ...
    public void testBackButton() {
        ...
        onData(is("Tokyo")).inAdapterView(withId(R.id.city_list)).perform(click());

        // Check that we are in ShowActivity
        onView(withId(R.id.button_back)).check(matches(isDisplayed()));

        // Click the BACK button
        onView(withId(R.id.button_back)).perform(click());

        // Check that we are back in MainActivity (e.g., ADD CITY button is visible)
        onView(withId(R.id.button_add)).check(matches(isDisplayed()));

        // Check that the BACK button from ShowActivity no longer exists
        onView(withId(R.id.button_back)).check(doesNotExist());
    }
}